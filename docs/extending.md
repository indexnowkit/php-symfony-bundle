...
| Clock | `indexnowkit.clock` | PSR-20 `Psr\Clock\ClockInterface` (default `Clock\SystemClock`) | | replace to move every time the graph reads at once: the throttle window, the `memory` debounce window, the time a submission record gets and the pre-flight decorator. `IndexNowKit\Testing\FrozenClock` in tests ([testing.md](testing.md)) |
