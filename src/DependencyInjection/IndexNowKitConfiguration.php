    ...
    public function build(DefinitionConfigurator $definition): void
    {
        ...
        $children
                ...
                ->arrayNode('engines')->defaultValue(['api'])
                    ->info('api = api.indexnow.org, shared by Yandex, Bing, Naver, Seznam, Yep, Internet Archive and Amazon. Name a single engine to target it, or give a full https endpoint URL.')
                    ->scalarPrototype()
                        ->validate()->ifTrue(self::literal(static fn(string $v): bool => !self::isKnownEngine($v)))->thenInvalid('Unknown IndexNow engine %s (api, yandex, bing, naver, seznam, yep, internetarchive, amazon, an alias from engine_aliases or an https URL).')->end()
                    ->end()
                ->end()
        ...
        $children
                ...
                ->arrayNode('logging')->addDefaultsIfNotSet()->children()
                    ...
                    ->end()
                ->end()->end()
                ->arrayNode('normalizer')->addDefaultsIfNotSet()->children()
                    ->booleanNode('strip_tracking_params')->defaultTrue()->info('Drop utm_*, gclid, fbclid, yclid, … from the query before de-duplication, debounce and submission (Url\CanonicalUrlNormalizer::TRACKING_PARAMS).')->end()
                    ->arrayNode('tracking_params')->scalarPrototype()->end()->info('More query parameters to drop: names ("ref") or prefixes ("mtm_*").')->end()
                    ->enumNode('trailing_slash')->values(Config::TRAILING_SLASH_MODES)->defaultValue(Config::DEFAULT_TRAILING_SLASH)->info('keep: as generated; add: a path without an extension ends with "/"; strip: no trailing "/" except the root. Match the site\'s canonical form.')->end()
                    ->booleanNode('sort_query')->defaultFalse()->info('Order the query parameters by name, so ?b=1&a=2 and ?a=2&b=1 are one URL.')->end()
                ->end()->end()
